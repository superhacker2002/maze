...
TEST_F(CaveAnswerTest, get_answer_usual_1) {
  ...
  std::vector<int> result =
      s21::getMazeAnswer(*test_answer_maze, {0, 0}, {3, 3});
...
TEST_F(CaveAnswerTest, get_answer_usual_2) {
  std::vector<int> correct_answer = {
      s21::RIGHT, s21::UP,   s21::UP,   s21::RIGHT, s21::RIGHT, s21::DOWN,
      s21::DOWN,  s21::LEFT, s21::DOWN, s21::LEFT,  s21::LEFT};
  auto it = correct_answer.begin();

  std::vector<int> result =
      s21::getMazeAnswer(*test_answer_maze, {0, 2}, {0, 3});
  for (auto& cell : result) {
    EXPECT_EQ(cell, *it);
    it++;
  }
...
TEST_F(CaveAnswerTest, get_answer_usual_3) {
  ...
  std::vector<int> result =
      s21::getMazeAnswer(*test_answer_maze, {2, 2}, {0, 1});
  for (auto& cell : result) {
    EXPECT_EQ(cell, *it);
    it++;
  }
