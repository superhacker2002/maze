...
constexpr std::array<s21::Coordinates, 4> DELTA = {
    {{0, -1}, {0, 1}, {1, 0}, {-1, 0}}};
 ...
 */
std::vector<int> getMazeAnswer(const s21::Maze& maze, Coordinates start,
                               Coordinates end) {
...
void checkCoordinates(int rows, int cols, Coordinates& coordinates) {
  if (coordinates.x < 0 || coordinates.y < 0 || coordinates.x > cols ||
      coordinates.y > rows) {
 ...
 */
bool correctCoordinates(AnswerData& data, Coordinates& current,
                        Coordinates& next, int shift) {
...
bool checkDownDirection(AnswerData& data, Coordinates& current, int rows) {
  return current.y < rows - 1 &&
         !data.maze_matrix.GetValue(current.y, current.x).bottom_wall;
}

bool checkUpDirection(AnswerData& data, Coordinates& current,
                      Coordinates& next) {
  return current.y > 0 &&
         !data.maze_matrix.GetValue(next.y, next.x).bottom_wall;
}

bool checkRightDirection(AnswerData& data, Coordinates& current, int cols) {
  return current.x < cols - 1 &&
         !data.maze_matrix.GetValue(current.y, current.x).right_wall;
}

bool checkLeftDirection(AnswerData& data, Coordinates& current,
                        Coordinates& next) {
 ...
 *
 */
std::vector<int> reestablishPath(s21::Matrix<Coordinates> answer_path,
                                 Coordinates& end) {
